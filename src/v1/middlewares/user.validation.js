...
export const signupValidation = [
    body('user_role_id', 'Introduce the role of the user').exists().isNumeric(),
    body('email', 'Email format incorrect').exists().isEmail(),
    body('password', 'Password length incorrect')
        .exists()
...
export const profileValidation = [
    ...
    body('date_birth', 'Introduce your date of birth')
        .exists()
        .isDate({ format: 'DD/MM/YYYY' }),
    body('profile_pic', 'Incorret format for profile image').isString(),
    body('contact_email', 'Incorrect format for contact email')
        .isString()
        .isEmail(),
